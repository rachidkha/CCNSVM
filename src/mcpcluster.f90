...
! --------------------------------------------------
SUBROUTINE hsvmclustermcp (delta, lam2, nobs, nvars, x,  y, tom, KK, jd, pf, pf2, dfmax, &
...
! - - - allocate variables - - -
      ...
      CALL hsvmclustermcppathc (delta, lam2, maj, nobs, nvars, x,  y, tom,  KK , ju, &
...
! - - - organize beta afterward - - -
      ...
      RETURN
END SUBROUTINE hsvmclustermcp
! --------------------------------------------------
SUBROUTINE hsvmclustermcppathc (delta, lam2, maj, nobs, nvars, x,  y,  tom, KK , ju, &
...
! --------------------------------------------------
      ...
      DOUBLE PRECISION, PARAMETER :: gam= 3.0d0
...
! --------- lambda loop ----------------------------
  ...
  !----------------------------------------------------------- 
                  ...
                  IF (Abs(u) < (v * gam * omega)) then
                     v = Abs (u) - v
                                         
                     IF (v > 0.0D0) THEN
                       b (k) = sign (v, u) / (omega - 1.0d0/gam )
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
     ...
     !!!!!!!!!!!!!!!!!!!!The secon final check
       ...
       END DO !end of the kmeans loop + gcd
          IF (ni > pmax) THEN
          jerr = - 10000 - l
          EXIT
         END IF 
...
END SUBROUTINE hsvmclustermcppathc
